...
**Goal:** Rebrand the existing Andromeda Night VS Code theme repo to "Tokyo Nebula" and port all 5 variants to a Zed extension that preserves colors 1:1.
...
- `themes/andromeda-night.json` → renamed to `themes/tokyo-nebula.json`; internal `name` field updated
- `themes/andromeda-night-italic.json` → renamed to `themes/tokyo-nebula-italic.json`; internal `name` updated
...
**Final (out-of-band):**
- Parent directory rename: `/Users/paolo/Tools/andromeda-night` → `/Users/paolo/Tools/tokyo-nebula`
...
```json
{
  ...
  "repository": {
    "type": "git",
    "url": "https://github.com/ni3rav/andromeda-night"
  },
  "bugs": {
    "url": "https://github.com/ni3rav/andromeda-night/issues"
  },
  "keywords": [
    "theme",
    "dark theme",
...
**Files:**
- Rename: `themes/andromeda-night.json` → `themes/tokyo-nebula.json` (update internal `name`)
- Rename: `themes/andromeda-night-italic.json` → `themes/tokyo-nebula-italic.json` (update internal `name`)
...
```bash
git mv themes/andromeda-night.json themes/tokyo-nebula.json
git mv themes/andromeda-night-italic.json themes/tokyo-nebula-italic.json
```

- [ ] **Step 2: Update the `name` field in `themes/tokyo-nebula.json`**

The first line of the file is `"name": "Andromeda Night",`. Edit that single occurrence so it reads:
...
Replace `"name": "Andromeda Night Italic",` with:
...
Replace `"name": "Andromeda Night Equalizer",` with:

```json
"name": "Tokyo Nebula Equalizer",
```

(If the source uses a different exact string like `"Equalizer"`, change it to `"Tokyo Nebula Equalizer"`. Confirm with `grep -n '"name"' themes/equalizer.json` first.)

- [ ] **Step 5: Update the `name` field in `themes/dusk.json`**

Replace `"name": "Andromeda Night Dusk",` (or whatever string is there) with:
...
name = "Tokyo Nebula"
version = "0.0.1"
description = "Tokyo Nebula — a dark theme family ported from the Andromeda Night VS Code theme. Five variants: classic, italic, equalizer, dusk, operator."
...
```
wrote /Users/paolo/Tools/andromeda-night/zed-extension/themes/tokyo-nebula.json
...
```bash
mv /Users/paolo/Tools/andromeda-night /Users/paolo/Tools/tokyo-nebula
```
...
If the repo on GitHub is also being renamed from `andromeda-night` to `tokyo-nebula`, do that in the GitHub UI, then:

```bash
git remote set-url origin git@github.com:ni3rav/tokyo-nebula.git
git remote -v
```
